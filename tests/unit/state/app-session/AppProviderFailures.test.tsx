...
import { useApp } from '@/state/app-session/appContextValue'
...
vi.mock('@/state/app-session/useAuthState', () => ({
  useAuthState: () => ({
    user,
...
vi.mock('@/persistence/sync/session/syncService', () => ({
  ...
  fetchProjects: vi.fn(),
  flushAllProjectSavesWithSync: vi.fn().mockResolvedValue(undefined),
...
import { AppProvider } from '@/state/app-session/AppProvider'
