...
vi.mock('@/engine/EngineAdapter', () => ({ getEngine: () => engine }))
vi.mock('@/persistence/sync/session/syncService', () => ({ loadFileWithSync: (...args: unknown[]) => loadFile(...args) }))
vi.mock('papaparse', () => ({
  default: {
...
import { computeDerivedTable } from '@/engine/materialization/derivedTableComputation'
import {
  computeDerivedVersionHash, computeSchemaFingerprint, computeSourceVersionHash,
} from '@/engine/materialization/cacheUtils'
