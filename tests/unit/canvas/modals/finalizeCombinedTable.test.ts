...
import { finalizeCombinedTable } from '@/canvas/modals/finalizeCombinedTable'
...
vi.mock('@/state/runtime/tableOperationCoordinator', async () => {
  const actual = await vi.importActual<typeof import('@/state/runtime/tableOperationCoordinator')>(
    '@/state/runtime/tableOperationCoordinator',
  )
  return {
    ...actual,
