...
vi.mock('@/dashboard/useProjectHealthMetrics', () => ({
  useProjectHealthMetrics: () => ({ totalTables: 1, totalRows: 2, totalColumns: 2, overallCompleteness: 100 }),
}))
vi.mock('@/dashboard/useDataQualityMetrics', () => ({
  useDataQualityMetrics: () => ({ tableMetrics: [{}] }),
}))
vi.mock('@/dashboard/useTopSuggestions', () => ({
  useTopSuggestions: () => ({ suggestions: [suggestion], isLoading: false }),
}))
vi.mock('@/dashboard/useLineageData', () => ({
  useLineageData: () => ({ nodes: [], edges: [] }),
}))
vi.mock('@/dashboard/components/LineageMiniMap', () => ({ LineageMiniMap: () => null }))
vi.mock('@/dashboard/components/TableStatsSection', () => ({ TableStatsSection: () => null }))
vi.mock('@/dashboard/components/ColumnStatComponents', () => ({ CompletenessBar: () => null }))
vi.mock('@/dashboard/components/QuickActionsSection', () => ({
