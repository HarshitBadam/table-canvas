...
import { saveProjectWithSync } from '@/persistence/sync/session/syncService'
import { useReportStore } from '@/report/reportStore'
import { getStorageScope } from '@/persistence/storage/storageScope'
import { markProjectExported } from './projectActivity'
...
export function useProjectExport(onImportComplete: () => void): ProjectExportState {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const { exportAndDownloadProject } = await import('@/persistence/import-export/export/exportService')
