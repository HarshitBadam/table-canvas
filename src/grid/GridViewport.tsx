    ...
    <div ref={containerRef} className="flex-1 overflow-auto select-none">
      <div style={{
        ...
        <div className="sticky top-0 z-20 flex border-b border-border table-header-bg" style={{ height: HEADER_HEIGHT }}>
          ...
          {isEditable && <div onClick={onAddColumn} className="flex items-center justify-center px-2 text-xs cursor-pointer border-l border-border text-text-tertiary hover:bg-green-50 dark:hover:bg-green-900/30 hover:text-green-600 dark:hover:text-green-400" style={{ width: 40, minWidth: 40 }} title="Add column">
        ...
        <div style={{ position: 'relative', height: rowVirtualizer.getTotalSize() }}>
          {virtualRows.map(virtualRow => {
            ...
            if (!row) {
              return <div key={`skeleton-${index}`} className="flex border-b border-border-subtle bg-surface animate-pulse" style={rowStyle}>
                <div className="sticky left-0 z-10 flex items-center justify-center px-3 text-xs border-r border-border-subtle text-text-tertiary bg-surface" style={{ width: 50, minWidth: 50 }}>{index + 1}</div>
