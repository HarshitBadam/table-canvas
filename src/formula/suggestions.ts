...
// Find a single column matching keywords
function findColumn(
  columns: ColumnInfo[],
  keywords: string[],
  preferredType?: string
...
const namePatterns: NamePattern[] = [
  ...
  // Post-tax / Net income patterns
  {
    ...
    generator: (_, columns) => {
      ...
      if (incomeCol && taxCol) {
        // Check if tax is likely a rate (0-1) or percentage (0-100) or absolute amount
        return {
  ...
  // Difference / Change patterns
  {
    ...
    generator: (_, columns) => {
      ...
      // Look for before/after or old/new patterns
      ...
      // If we have at least 2 numeric columns, suggest difference
  ...
  // Percentage patterns
  {
    ...
    generator: (_columnName, columns) => {
      ...
      if (numericCols.length >= 2) {
        // Find a "total" column for percentage calculation
        ...
        // If no obvious total, suggest ratio of first two numeric columns
        return {
  ...
  // Is empty / Has value
  {
    ...
    generator: (columnName, columns) => {
      ...
      // Try to find a column that commonly has missing values
 ...
 */
export function getColumnAutocomplete(
  columns: ColumnInfo[],
  filter?: string
): AutocompleteItem[] {
  ...
  if (filter) {
    ...
    filtered = columns.filter(col => 
      ...
      col.id.toLowerCase().includes(lowerFilter)
    )
  }
...
/**
 * Get autocomplete items for functions
 */
export function getFunctionAutocomplete(
  filter?: string
): AutocompleteItem[] {
  ...
  if (filter) {
    ...
    functions = functions.filter(fn => 
      ...
      fn.description.toLowerCase().includes(lowerFilter)
    )
  }
...
/**
 * Get all autocomplete items (columns and functions)
 */
export function getAutocompleteItems(
  columns: ColumnInfo[],
  filter?: string,
  type?: 'column' | 'function' | 'all'
): AutocompleteItem[] {
  const items: AutocompleteItem[] = []

  if (type === 'column' || type === 'all' || !type) {
    items.push(...getColumnAutocomplete(columns, filter))
  }

  if (type === 'function' || type === 'all' || !type) {
    items.push(...getFunctionAutocomplete(filter))
  }
