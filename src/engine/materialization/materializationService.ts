import { getComputationOrder } from '../graph/dependencyGraph'
import { useProjectStore } from '@/state/projectStore'
import { useDataStore, TableRow } from '@/state/dataStore'
import { loadFileWithSync } from '@/persistence/sync/session/syncService'
import {
...
import { waitForTableOperation } from '@/state/runtime/tableOperationCoordinator'
...
async function loadSourceTable(
  ...
  try {
    ...
    if (snapshot.node.plan.fileRef) {
      ...
      if (fileData) {
        const { parseFileData } = await import('../parsing/fileParsers')
